...
/**
 * Returns distance and the specific edge segment that is closest.
 */
function getNearestEdgeInfo(point: Point, polygon: Polygon): { distance: number, edge: Point[] } {
  let minDist = Infinity;
  let bestEdge: Point[] = [];
  ...
  for (let i = 0, j = polygon.length - 1; i < polygon.length; j = i++) {
    ...
    if (d < minDist) {
        minDist = d;
        bestEdge = [polygon[i], polygon[j]];
    }
  }
  return { distance: minDist, edge: bestEdge };
}
...
export function calculateLabelPlacement(
  ...
  for (const seedPoint of finalSeeds) {
    ...
    const collisionEdges: Point[][] = [];
    
    for (const p of smoothedSpine) {
        const { distance, edge } = getNearestEdgeInfo(p, polygon);
        const c = distance;
  ...
  if (!best) {
     ...
     // Generate fallback collision points and edges
     const fallbackCols: Point[] = [];
     const fallbackEdges: Point[][] = [];
     const steps = 10;
     for(let i=0; i<=steps; i++){
         const t = i/steps;
         const p = { x: start.x + (end.x - start.x)*t, y: start.y + (end.y - start.y)*t };
         const { distance, edge } = getNearestEdgeInfo(p, polygon);
         if (distance < textHeight * 0.5) {
             fallbackCols.push(p);
             if (edge.length === 2) fallbackEdges.push(edge);
         }
     }
